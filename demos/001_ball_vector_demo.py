...
sliders = {
    'x_bias': {'pos': (50 * ratio, 80 * ratio), 'min': -1, 'max': 2},
}
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.button == 4:  # Scroll up to increase cyan line length
                x_bias += 0.1
                if x_bias > 2:  # Ensure there's a maximum length
                    x_bias = 2
            elif event.button == 5:  # Scroll down to decrease cyan line length
                x_bias -= 0.1
                if x_bias < -1:  # Ensure there's a minimum length
                    x_bias = -1
    ...
    draw_vector_with_arrowhead(mouse_x, mouse_y, cyan_end_x, cyan_end_y, RED, ARROW_THICKNESS, x_bias)
    ...
    if show_components:
        ...
        draw_vector_with_arrowhead(mouse_x, mouse_y, cyan_end_x, mouse_y, CYAN, ARROW_THICKNESS, x_bias)
    ...
    for key, slider in sliders.items():
        ...
        slider['handle'].x = slider['pos'][0] + int((x_bias - sliders[key]['min']) / (sliders[key]['max'] - sliders[key]['min']) * 200 * ratio)
